#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""bank_reconcile_engine 专业对账测试：方向镜像/逐笔层级/四分类/调节表/红旗"""
import sys
import tempfile
...
from core.bank_reconcile_engine import (
    JOURNAL, BANK_STATEMENT, detect_book_type, auto_map_columns,
    normalize_to_std, filter_bank_account, tie_out_balance,
    run_bank_reconciliation, export_reconciliation_outputs, detect_duplicates,
    CAT_ENT_RECV, CAT_BANK_RECV, CAT_REVIEW)
...
book = pd.DataFrame({
    "序号": [1, 2, 3, 4, 5],
    "月": [3, 3, 3, 3, 3],
    "日期": ["2026-03-01", "2026-03-02", "2026-03-05", "2026-03-28", "2026-03-10"],
    "凭证号": ["记-001", "记-002", "记-003", "记-004", "记-005"],
    "摘要": ["收货款-甲公司", "付货款-乙公司", "收货款-丙公司", "收货款-丁公司", "付费用"],
    "借方金额": [10000, 0, 8000, 6000, 0],
    "贷方金额": [0, 5000, 0, 0, 999],
    "银行账号": ["农行5927"] * 5,
})
# ── 银行流水（所有银行，" 摘要"列带前导空格） ──────────────────
bank = pd.DataFrame({
    ...
    "银行账号": ["农行5927", "农行5927", "农行5927", "农行5927", "农行5927",
                 "工行1101", "农行5927", "农行5927", "农行5927"],
    "日期": ["2026-03-01", "2026-03-03", "2026-03-05", "2026-03-05", "2026-03-29",
             "2026-03-01", "2026-03-15", "2026-03-16", "2026-03-16"],
    " 摘要": ["甲公司货款", "付乙公司货款", "丙公司款", "丙公司款", "利息收入",
              "其他行流水", "支取现金", "收甲转账", "付戊转账"],
    "借方（支取）": [0, 5000, 0, 0, 0, 0, 60000, 0, 50000],
    "贷方（收入）": [10000, 0, 5000, 3000, 88.8, 77777, 0, 50000, 0],
})
...
assert len(kstd2) == 8 and "农行5927" in note and "8" in note, note
ok("多账户流水过滤")

# 5) 勾稽：期初0 + Σnet(18001) = 期末18001
t = tie_out_balance(bstd, opening=0, closing=18001)
assert t["checked"] and t["balanced"], t
ok("期初+发生额=期末勾稽")

# 6) 完整对账
res = run_bank_reconciliation(book, bank, {
    "book_file": "序时账1.xlsx", "bank_file": "银行流水1.xlsx",
    "book_closing": 10000, "bank_closing": 4088.8})
s = res["stats"]
...
# 7) 未匹配分类：丁公司6000(3-28)→企收银未收；利息88.8→银收企未收+利息标记；
#    付费用999(3-10)/现金60000(3-15)→待人工核查（不得洗白成未达）
...
ok("未达四分类 + 利息专项标记 + 兜底待人工核查")

# 8) 余额调节表：10000+88.8 = 4088.8+6000 = 10088.8，调节差异=0
recon = {r["项目"]: r["金额"] for r in res["balance_reconciliation"]}
assert abs(recon["调节后企业账面余额"] - 10088.8) < 0.01
assert abs(recon["调节差异"]) < 0.01, recon
ok("银行存款余额调节表（四方平衡）")

# 9) 红旗：一收一付同额(50000/3-16) + 大额现金(60000)
flag_types = {f["type"] for f in res["red_flags"]}
assert "一收一付同额" in flag_types and "大额现金" in flag_types, flag_types
ok("12号文红旗规则（一收一付同额/大额现金）")
...
# 11) 交付物导出
out = Path(tempfile.mkdtemp(prefix="reconcile_test_"))
files = export_reconciliation_outputs(res, out)
assert len(files) == 5 and all((out / f).exists() for f in files), files
detail = pd.read_excel(out / "逐笔对账明细底稿.xlsx")
assert "状态" in detail.columns and (detail["状态"] == "已核对").sum() == 3
ok("5 项专业交付物导出")

print("\n全部通过：bank_reconcile_engine 专业对账链路")
